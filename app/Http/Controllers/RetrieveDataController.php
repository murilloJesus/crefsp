...
class RetrieveDataController extends Controller
{
    ...
    public function recursivePaginas($id, $recursive = 3, $get_itens = false, $data = false)
    {
        if($data) $root = $data;
        else $root = Pagina::find($id);
    ...
    public function recursiveFilhos($id, $recursive = -1)
    {
        $descendants = Pagina::where(['parent_id' => $id, 'status' => true])->orderBy('indice', 'asc')->orderBy('id', 'asc')->get();
        ...
        foreach ($descendants as $pagina) {
            ...
            $pagina = $this->recursivePaginas($id, $recursive-1, false, $pagina);
    ...
    public function recursiveItens($id, $select = '*')
    {
        $itens = Item::where('pagina_id', '=', $id)->where('status', true)
                        ->orderBy('indice', 'asc')->orderBy('datayy', 'desc')->orderBy('name', 'asc')->orderBy('data', 'desc')->orderBy('id', 'asc')
                        ->get($select);

        return $this->makeItens($itens);
    ...
    {
        if ($initial) {
            $itens = Item::where('pagina_id', '=', $id)->where('status', true)->where('destaque', 0)
                        ->take(100)
                        ->orderBy('destaque', 'asc')->orderBy('data', 'desc')->orderBy('indice', 'asc')->orderBy('datayy', 'desc')->orderBy('name', 'asc')->orderBy('data', 'desc')
                        ->select('id', 'name',  'type', 'descricao', 'categoria', 'destaque', 'destaque_pequeno', 'destaque_medio', 'destaque_grande', 'imagem', 'created_at', 'data')
                        ->get();

            $destaque = Item::where('pagina_id', '=', $id)->where('status', true)->where('destaque', 1)
                        ->orderBy('destaque', 'asc')->orderBy('data', 'desc')->orderBy('indice', 'asc')->orderBy('datayy', 'desc')->orderBy('name', 'asc')->orderBy('data', 'desc')
                        ->select('id', 'name',  'type', 'descricao', 'categoria', 'destaque', 'destaque_pequeno', 'destaque_medio', 'destaque_grande', 'imagem', 'created_at', 'data')
                        ->get();

            $itens = $itens->merge($destaque);
    ...
    {
        $itens[0] = Item::where('type', 'noticia')->where('status', true)->take(7)->orderBy('created_at', 'desc')->get();
    ...
    public function getItems($id)
    {
        switch ($id){
            ...
            case 373:
            case 375:
                return json_encode($this->recursiveItens($id, $this->getType('link')));
    ...
    private function getType($type){
        switch($type){
            case 'link':
                return ['id', 'type', 'name', 'source', 'href', 'descricao', 'categoria', 'datamm', 'datayy', 'data' ];
        }
    }
